...
class TestErrorHandlingConsistency:
    ...
    @given(exception_class=exception_classes, message=error_messages, context=context_dicts)
    ...
    @given(exception_class=exception_classes, message=error_messages, context=context_dicts)
    def test_context_preserved_in_string_representation(self, exception_class, message, context):
        """
        ...
        if context:
            assert "Context:" in error_str or any(str(k) in error_str for k in context.keys())
    ...
    def test_context_immutability_after_creation(self, exception_class, message, context):
        """
    ...
    def test_specific_exception_types_for_specific_errors(self, message, context):
        ...
        errors = [config_error, element_error, value_error, validation_error, resource_error]
        for error in errors:
            assert error.message == message
            assert error.context == context
