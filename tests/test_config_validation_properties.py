...
valid_retry_attempts = st.integers(min_value=MIN_RETRY_ATTEMPTS, max_value=MAX_RETRY_ATTEMPTS)
valid_fuzzy_thresholds = st.integers(min_value=MIN_FUZZY_MATCH_THRESHOLD, max_value=MAX_FUZZY_MATCH_THRESHOLD)
valid_exponential_bases = st.floats(min_value=1.1, max_value=10.0, allow_nan=False, allow_infinity=False)

# Strategies for invalid configuration values
invalid_log_levels = st.text().filter(lambda x: x not in [level.value for level in LogLevel])
invalid_log_formats = st.text().filter(lambda x: x not in [fmt.value for fmt in LogFormat])
...
invalid_exponential_bases = st.floats(max_value=1.0, allow_nan=False, allow_infinity=False)
...
class TestConfigurationValidation:
    ...
    def test_llm_config_rejects_invalid_max_retries(self, max_retries):
        ...
        error = exc_info.value
        assert "max_retries" in error.message.lower() or "retry" in error.message.lower()
    ...
    @given(
        ...
        max_delay=valid_positive_ints
    )
    def test_llm_config_rejects_max_delay_less_than_base_delay(self, base_delay, max_delay):
        """
        ...
        config = LLMConfig(retry_base_delay=float(base_delay), retry_max_delay=float(max_delay))
    ...
    def test_performance_config_rejects_non_positive_connection_pool_size(self, pool_size):
        """
    ...
    def test_valid_logging_config_passes_validation(self, level, format, max_file_size, backup_count):
        """
    ...
    def test_valid_llm_config_passes_validation(self, temperature, timeout, max_retries, base_delay, exponential_base):
        """
    ...
    def test_configuration_validates_all_subconfigs(self, level, temperature, threshold):
        """
